...
def make_groups(df: pd.DataFrame, group_column: str = "PE - PLI #") -> np.ndarray:
    preferred = df[group_column] if group_column in df.columns else pd.Series([""] * len(df))
    product_event = df["Product Event ID"] if "Product Event ID" in df.columns else pd.Series([""] * len(df))
    pli = df["PE - PLI #"] if "PE - PLI #" in df.columns else pd.Series(np.arange(len(df)).astype(str))
    preferred = preferred.fillna("").astype(str).str.strip()
    product_event = product_event.fillna("").astype(str).str.strip()
    pli = pli.fillna("").astype(str).str.strip()
